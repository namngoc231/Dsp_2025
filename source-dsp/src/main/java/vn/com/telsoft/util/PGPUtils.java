...
public class PGPUtils {
    ...
    public static PGPPublicKey readPublicKey(InputStream in, KeyFingerPrintCalculator keyFingerPrintCalculator)
            throws IOException, PGPException {

        PGPPublicKeyRingCollection keyRingCollection = new PGPPublicKeyRingCollection(PGPUtil.getDecoderStream(in), keyFingerPrintCalculator);
    ...
    @SuppressWarnings("unchecked")
    public static PGPSecretKey readSecretKey(InputStream in, KeyFingerPrintCalculator keyFingerPrintCalculator)
            throws IOException, PGPException {

        PGPSecretKeyRingCollection keyRingCollection = new PGPSecretKeyRingCollection(PGPUtil.getDecoderStream(in), keyFingerPrintCalculator);
    ...
    /**
     * Load a secret key ring collection sender keyIn and find the private key corresponding to
     * keyID if it exists.
     *
     ...
     */
    public static PGPPrivateKey findPrivateKey(InputStream keyIn, long keyID, char[] pass, KeyFingerPrintCalculator keyFingerPrintCalculator)
            throws IOException, PGPException, NoSuchProviderException {
        PGPSecretKeyRingCollection pgpSec = new PGPSecretKeyRingCollection(PGPUtil.getDecoderStream(keyIn), keyFingerPrintCalculator);
...
//            pbe = it.next();
//
//            sKey = findPrivateKey(keyIn, pbe.getKeyID(), passwd, keyFingerPrintCalculator);
//        }
//
//        if (sKey == null) {
...
//
//        PGPObjectFactory plainFact = new PGPObjectFactory(clear, keyFingerPrintCalculator);
//
//        Object message = plainFact.nextObject();
//
//        if (message instanceof PGPCompressedData) {
//            PGPCompressedData cData = (PGPCompressedData) message;
//            PGPObjectFactory pgpFact = new PGPObjectFactory(cData.getDataStream(), keyFingerPrintCalculator);
//
...
//    }
    ...
    public static boolean verifyFile(
        ...
        PGPObjectFactory pgpFact = new PGPObjectFactory(in, keyFingerPrintCalculator);
        PGPCompressedData c1 = (PGPCompressedData) pgpFact.nextObject();

        pgpFact = new PGPObjectFactory(c1.getDataStream(), keyFingerPrintCalculator);
        ...
        int ch;
        PGPPublicKeyRingCollection pgpRing = new PGPPublicKeyRingCollection(PGPUtil.getDecoderStream(keyIn), keyFingerPrintCalculator);
    ...
    public static boolean encrypt(String publicKey, OutputStream out, InputStream in) throws Exception {
        KeyFingerPrintCalculator keyFingerPrintCalculator = new BcKeyFingerprintCalculator();
        ByteArrayInputStream key = new ByteArrayInputStream(publicKey.getBytes());
        PGPUtils.encryptFile(out, in, PGPUtils.readPublicKey(key, keyFingerPrintCalculator), true, true, "");
        return true;
    }
